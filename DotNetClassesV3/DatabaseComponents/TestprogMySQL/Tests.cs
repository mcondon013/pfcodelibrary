    ...
    public class Tests
    {
        ...
        public static void ConnectionStringTest(MainForm frm)
        {
            ...
            try
            {
                ...
                _msg.Append("\r\n");
                _msg.Append("ServerName = ");
                _msg.Append(db.ServerName);
                _msg.Append("\r\n");
                _msg.Append("DatabaseName = ");
                _msg.Append(db.DatabaseName);
                _msg.Append("\r\n");
                _msg.Append("PortNumber = ");
                _msg.Append(db.PortNumber);
                _msg.Append("\r\n");
                _msg.Append("Username = ");
                _msg.Append(db.Username);
                _msg.Append("\r\n");
                _msg.Append("Password = ");
                _msg.Append(db.Password);
                _msg.Append("\r\n");
                Program._messageLog.WriteLine(_msg.ToString());
                ...
                if (db.IsConnected)
                {
                    ...
                    _msg.Append("ConnectionTimeout = ");
                    _msg.Append(db.Connection.ConnectionTimeout.ToString());
                    _msg.Append("\r\n");
                    _msg.Append("DataSource = ");
                    _msg.Append(db.Connection.DataSource);
                    _msg.Append("\r\n");
                    _msg.Append("ServerVersion = ");
                    _msg.Append(db.Connection.ServerVersion);
                    _msg.Append("\r\n");
                    _msg.Append("UseCompression = ");
                    MySqlConnection connTemp = (MySqlConnection)db.Connection;
                    _msg.Append(connTemp.UseCompression.ToString());
                    _msg.Append("\r\n");
                    _msg.Append("State = ");
        ...
        public static void CreateTableTest(MainForm frm)
        {
            ...
            try
            {
                mysql.ServerName = frm.txtServerName.Text;
                mysql.DatabaseName = frm.txtDatabaseName.Text;
                mysql.Username = frm.txtUsername.Text;
                mysql.Password = frm.txtPassword.Text;
                mysql.PortNumber = frm.txtPortNumber.Text;
                ...
                //{
                //    schemaName = mysql.DatabaseName;
                //    tabName = parsedTableName[0];
                //}
                //else
                //    ;

                if (mysql.TableExists(schemaName, tableName))
                {
                    bool dropped = mysql.DropTable(schemaName, tableName);
                    if (dropped == false)
                    {
                        _msg.Length = 0;
                ...
                if (mysql.TableExists(schemaName, tableName2))
                {
                    bool dropped = mysql.DropTable(schemaName, tableName2);
                    if (dropped == false)
                    {
                        _msg.Length = 0;
                        _msg.Append("Unable to drop table ");
                ...
                IDatabaseProvider db = (IDatabaseProvider)mysql;
                ...
                dataTypesToInclude.Add(new KeyValuePair<string, string>("System.Char[]", "ABCDEFGH"));  //MySQL mishandles char[]: will be reset to a byte[] by PFMYSQL routine that imports data.
                ...
                dataTypesToInclude.Add(new KeyValuePair<string, string>("System.Guid", "58a4a08d-6101-4393-86dc-b2a8db46ec0f")); //MySql cannot handle guid datatype; type will be changed to string for output purposes.
                ...
                dt.Columns.Add(f4);
                DataColumn f4a = new DataColumn("F4A BEST", Type.GetType("System.UInt16"));
                dt.Columns.Add(f4a);
                ...
                dt.Columns.Add(f12);
                DataColumn f13 = new DataColumn("F13 LAST", Type.GetType("System.Guid"));
                dt.Columns.Add(f13);

                createScript = string.Empty;
                bool result = mysql.CreateTable(dt, out createScript);
                */
        ...
        public static void DataReaderTest(MainForm frm)
        {
            ...
            try
            {
                ...
                db.returnResultAsString += new PFMySQL.ResultAsStringDelegate(OutputResultsToFile);
                if (_textFile.FileIsOpen)
                    _textFile.CloseFile();
                _textFile.OpenFile(@"c:\temp\ReaderDelimitedTestExtract.txt", PFFileOpenOperation.OpenFileForWrite);
                sw.Start();
                MySqlDataReader rdr = (MySqlDataReader)db.RunQueryDataReader();
                ...
                sw.Start();
                rdr = (MySqlDataReader)db.RunQueryDataReader();
        ...
        public static void DataTableTest(MainForm frm)
        {
            ...
            try
            {
                ...
                db.returnResult += new PFMySQL.ResultDelegate(OutputResults);
        ...
        public static void ImportDataTableTest(MainForm frm)
        {
            ...
            try
            {
                ...
                StringBuilder sql = new StringBuilder();
                DataTable dt = frm.keyValsDataSet.Tables["SAKILA.KeyValTable"];
