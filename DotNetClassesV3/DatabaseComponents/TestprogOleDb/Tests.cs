    ...
    public class Tests
    {
        ...
        public static void RunConnectionTest(MainForm frm)
        {
            ...
            try
            {
                ...
                _msg.Append("\r\n");
                _msg.Append("Provider is  ");
                _msg.Append(db.Provider);
                _msg.Append("\r\n");
                _msg.Append("DataSource is  ");
                _msg.Append(db.DataSource);
                _msg.Append("\r\n");
                _msg.Append("UDLFIleName is  ");
                _msg.Append(db.UDLFileName);
                _msg.Append("\r\n");
                _msg.Append("PersistSecurityInfo is  ");
                _msg.Append(db.PersistSecurityInfo.ToString());
                _msg.Append("\r\n");
                _msg.Append("OleDbServices is  ");
                _msg.Append(db.OleDbServices.ToString());
                _msg.Append("\r\n");
        ...
        public static void RunDataReaderTest(MainForm frm)
        {
            ...
            try
            {
                ...
                db.returnResult += new PFOleDb.ResultDelegate(OutputResults);
                ...
                db.returnResultAsString += new PFOleDb.ResultAsStringDelegate(OutputResultsToFile);
                if (_textFile.FileIsOpen)
                    _textFile.CloseFile();
                _textFile.OpenFile(@"c:\temp\OleDbRdrDelimited.txt", PFFileOpenOperation.OpenFileForWrite);
                ...
                if (_textFile.FileIsOpen)
                    _textFile.CloseFile();
                _textFile.OpenFile(@"c:\temp\OleDbRdrFixedLength.txt", PFFileOpenOperation.OpenFileForWrite);
                rdr = (OleDbDataReader)db.RunQueryDataReader();
                ...
                rdr = (OleDbDataReader)db.RunQueryDataReader();
                db.SaveDataReaderToXmlFile(rdr, @"c:\temp\OleDbTestrdr.xml");
                rdr.Close();
                rdr = (OleDbDataReader)db.RunQueryDataReader();
                db.SaveDataReaderWithSchemaToXmlFile(rdr, @"c:\temp\OleDbTestrdrplus.xml");
                rdr.Close();
                rdr = (OleDbDataReader)db.RunQueryDataReader();
                db.SaveDataReaderToXmlSchemaFile(rdr, @"c:\temp\OleDbTestrdr.xsd");
                rdr.Close();
        ...
        public static void RunDataTableTest(MainForm frm)
        {
            ...
            try
            {
                ...
                if (_textFile.FileIsOpen)
                    _textFile.CloseFile();
                _textFile.OpenFile(@"c:\temp\OleDbTabDelimited.txt", PFFileOpenOperation.OpenFileForWrite);
                tab = db.RunQueryDataTable();
                db.ExtractDelimitedDataFromTable(tab, ",", "\r\n", true);

                if (_textFile.FileIsOpen)
                    _textFile.CloseFile();
                _textFile.OpenFile(@"c:\temp\OleDbTabFixedLength.txt", PFFileOpenOperation.OpenFileForWrite);
                tab = db.RunQueryDataTable();
                ...
                tab = db.RunQueryDataTable();
                db.SaveDataTableToXmlFile(tab, @"c:\temp\OleDbTestTab.xml");
                tab = db.RunQueryDataTable();
                db.SaveDataTableWithSchemaToXmlFile(tab, @"c:\temp\OleDbTestTabPlus.xml");
                tab = db.RunQueryDataTable();
                db.SaveDataTableToXmlSchemaFile(tab, @"c:\temp\OleDbTestTab.xsd");
        ...
        public static void RunDataSetTest(MainForm frm)
        {
            ...
            try
            {
                ...
                if (_textFile.FileIsOpen)
                    _textFile.CloseFile();
                _textFile.OpenFile(@"c:\temp\OleDbDsDelimited.txt", PFFileOpenOperation.OpenFileForWrite);
                ds = db.RunQueryDataSet();
                db.ExtractDelimitedDataFromDataSet(ds, ",", "\r\n", true);

                if (_textFile.FileIsOpen)
                    _textFile.CloseFile();
                _textFile.OpenFile(@"c:\temp\OleDbDsFixedLength.txt", PFFileOpenOperation.OpenFileForWrite);
                ds = db.RunQueryDataSet();
                ...
                ds = db.RunQueryDataSet();
                db.SaveDataSetToXmlFile(ds, @"c:\temp\OleDbTestDs.xml");
                ds = db.RunQueryDataSet();
                db.SaveDataSetWithSchemaToXmlFile(ds, @"c:\temp\OleDbTestDsPlus.xml");
                ds = db.RunQueryDataSet();
                db.SaveDataSetToXmlSchemaFile(ds, @"c:\temp\OleDbTestDs.xsd");
        ...
        {
            PFOleDb oledb = new PFOleDb();
            IDatabaseProvider db = oledb;
            ...
            try
            {
                ...
                {
                    _msg.Length = 0;
                    _msg.Append("Create table does not work with Informix driver. Driver does not have capability of retrieving schema information.");
                ...
                PFTableBuilder tabBuilder = new PFTableBuilder(DatabasePlatform.OLEDB);
                List<DataTypeMapping> dataTypeMappings = tabBuilder.GetGenericDataTypeMappingsEx(db.ConnectionString);
                //List<DataTypeMapping> dataTypeMappings = tabBuilder.GetGenericDataTypeMappingsAll(db.ConnectionString);
                ...
                unitTestDt02.SetDataTypeOptions("System.String", false, true, 75000);  //will not work for MsOracle driver


                //MsOracle driver will fail to create table for strings longer than 4000 bytes, more than one byte array or a char array.
                ...
                bool result = false;
                if (frm.chkUseOleDbBuilder.Checked)
                {
                    dt = unitTestDt01.GetDataTableFromTableColumns(true);
                    result = oledb.CreateTableUsingOleDbSyntax(dt, out createScript);
                    dt2 = unitTestDt02.GetDataTableFromTableColumns(true);
                    result = oledb.CreateTableUsingOleDbSyntax(dt2, out createScript);
                }
                else if (frm.chkUseCustomSQL.Checked)
                {
                    result = oledb.CreateTableUsingCustomScript(frm.txtSqlQuery.Text);
                ...
                if (frm.chkUseOleDbBuilder.Checked)
                {
                    oledb.ImportDataFromDataTable(dt);
                }
                else if (frm.chkUseCustomSQL.Checked)
                {
                    _msg.Length = 0;
                ...
                if (frm.chkUseOleDbBuilder.Checked)
                {
                    oledb.ImportDataFromDataTable(dt2);
                }
                else if (frm.chkUseCustomSQL.Checked)
                {
                    _msg.Length = 0;
