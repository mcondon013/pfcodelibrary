...
namespace PFDataAccessObjects
{
    ...
    /// </summary>
    public class PFOleDb : IDatabaseProvider 
    {
        ...
        private string _udlFileName = string.Empty;
...
#pragma warning restore 1591
        ...
        public string ConnectionString
        {
            ...
            set
            {
                ...
                _dbPlatform = GetDatabasePlatform();
            }
        }
        ...
        {
            OleDbConnectionStringBuilder oledbConnBuilder;
            oledbConnBuilder = new OleDbConnectionStringBuilder(_connectionString.Trim());
            _provider = oledbConnBuilder.Provider;
            _dataSource = oledbConnBuilder.DataSource;
            _udlFileName = oledbConnBuilder.FileName;
            _persistSecurityInfo = oledbConnBuilder.PersistSecurityInfo;
            _oleDbServices = oledbConnBuilder.OleDbServices;
            GetConnectionStringKeyVals();
        }

        private void BuildConnectionString()
        {
            OleDbConnectionStringBuilder oledbConnBuilder = new OleDbConnectionStringBuilder();

            if (_provider != string.Empty)
                oledbConnBuilder.Provider = _provider;
            if (_dataSource != string.Empty)
                oledbConnBuilder.DataSource = _dataSource;
            if (_udlFileName != string.Empty)
                oledbConnBuilder.FileName = _udlFileName;
            oledbConnBuilder.PersistSecurityInfo = _persistSecurityInfo;
            oledbConnBuilder.OleDbServices = _oleDbServices;
            _connectionString = oledbConnBuilder.ToString();
        }
        ...
        private PFKeyValueList<string, string> GetConnectionStringKeyVals()
        {
            ...
            foreach (string key in oledbConnBuilder.Keys)
            {
                _connectionStringKeyVals.Add(new stKeyValuePair<string, string>(key, oledbConnBuilder[key].ToString()));
            }
        ...
        public bool CreateTable(DataTable dt, out string createScript)
        {
            ...
            int rowsAffected = 0;
            DatabasePlatform dbplat = DatabasePlatform.OLEDB;
            ...
            try
            {
                ...
                if (dbplat == DatabasePlatform.MSAccess)
                {
                    ...
                    OleDbConnectionStringBuilder cnb = new OleDbConnectionStringBuilder(this.ConnectionString);
                    string fileName = cnb["Data Source"].ToString();
        ...
        public bool CreateTableUsingOleDbSyntax(DataTable dt, out string createScript)
        {
            bool bSuccess = true;
            string sqlScript = string.Empty;
            int rowsAffected = 0;
            ...
            try
            {
                if (this.ConnectionString.ToLower().Contains("provider=microsoft.ace.oledb".ToLower())
                    || this.ConnectionString.ToLower().Contains("provider=microsoft.jet.oledb".ToLower()))
                {
        ...
        public DatabasePlatform GetDatabasePlatform()
        {
            ...
            else if (this.ConnectionString.ToLower().Contains("provider=oraoledb.oracle".ToLower()))
            {
                dbplat = DatabasePlatform.OracleNative;
            }
            else if (this.ConnectionString.ToLower().Contains("provider=msdaora".ToLower()))
            {
                dbplat = DatabasePlatform.MSOracle;
            }
            else if (this.ConnectionString.ToLower().Contains("provider=sqloledb".ToLower()))
            {
                dbplat = DatabasePlatform.MSSQLServer;
            }
            else if (this.ConnectionString.ToLower().Contains("provider=ibmdadb2".ToLower()))
            {
                dbplat = DatabasePlatform.DB2;
            }
            else if (this.ConnectionString.ToLower().Contains("provider=microsoft.sqlserver.ce.oledb.3.5".ToLower()))
            {
                dbplat = DatabasePlatform.SQLServerCE35;
            }
            else if (this.ConnectionString.ToLower().Contains("provider=aseoledb"))
            {
                dbplat = DatabasePlatform.Sybase;
            }
            else if (this.ConnectionString.ToLower().Contains("provider=saoledb"))
            {
        ...
        public string GetFullTableName(DataRow dr)
        {
            ...
            try
            {
                if (dr["TABLE_SCHEMA"].ToString().Trim().Length > 0)
                    tabName = dr["TABLE_SCHEMA"].ToString() + "." + dr["TABLE_NAME"].ToString();
                else
        ...
        public bool TableExists(string catalogName, string schemaName, string tableName)
        {
            ...
            string catColName = "TABLE_NAME";
            string schemaColName = "TABLE_SCHEMA";
            string tabColName = "TABLE_CATALOG";
            string temp = string.Empty;

            if (this.ConnectionString.ToLower().ToLower().Contains("ifxoledbc"))
            {
                _msg.Length = 0;
                _msg.Append("Function to check if table exists does not work with Informix OLEDB provider. Provider does not have capability of retrieving schema information. Use Informix .NET Provider or Informix ODBC driver instead.");
        ...
        public bool DropTable(string catalogName, string schemaName, string tableName)
        {
            ...
            {
                _msg.Length = 0;
                _msg.Append("Drop table operation invalid or syntax incorrect for this OLEDB driver: Error Message: ");
        ...
        public DbDataReader RunQueryDataReader(string sqlQuery, CommandType pCommandType)
        {
            ...
            _sqlQuery = sqlQuery;
            OleDbDataReader rdr = _cmd.ExecuteReader();
            return rdr;
        }
        ...
        public void ImportDataFromDataTable(DataTable dt, int updateBatchSize)
        {
            ...
            //sql.Append(";");
            OleDbCommand cmd = new OleDbCommand(sql.ToString(), _conn);
            cmd.CommandType = CommandType.Text;
            cmd.CommandTimeout = _commandTimeout;
            DataTable dbTable = dt.Clone();
            OleDbDataAdapter da = new OleDbDataAdapter(cmd);

            if (this.ConnectionString.ToLower().Contains("sqlserver.ce.oledb"))
            {
                _msg.Length = 0;
                _msg.Append("Do not use the SQL Server CE OLEDB provider to import data from a DataTable. Use the .NET provider for SQL Server CE instead. \r\n");
                _msg.Append("Connection string for this connection is \r\n");
                _msg.Append(this.ConnectionString);
                throw new DataException(_msg.ToString());
            }
            ...
            if (da.InsertCommand.CommandText.ToLower().Contains("default values"))
            {
                //this is considered an error: occurs with the SQL CE 3.5 provider
                string insertScript = BuildInsertScript(dt);
                da.InsertCommand = null;
                da.InsertCommand = new OleDbCommand(insertScript, _conn);
            }
            ...
            {
                FixColumnDataValues(dt, dbTable, supportedTypes);
            }
        ...
        private string BuildInsertScript(DataTable dt)
        {
            ...
            for (colInx = 0; colInx <= maxColInx; colInx++)
            {
                ...
                insertScript.Append(dc.ColumnName);
                if (colInx < maxColInx)
                    insertScript.Append(", ");
                else
                    //last column
            ...
            for (colInx = 0; colInx <= maxColInx; colInx++)
            {
                if (colInx == 0)
                    insertScript.Append("(");
                insertScript.Append("?");
                //insertScript.Append("@p");
                //insertScript.Append((colInx + 1).ToString());
                if (colInx < maxColInx)
                    insertScript.Append(", ");
                else
                    //last column
