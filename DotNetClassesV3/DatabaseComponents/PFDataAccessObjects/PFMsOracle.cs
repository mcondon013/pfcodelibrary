...
namespace PFDataAccessObjects
{
    ...
    public class PFMsOracle : IDatabaseProvider 
    {
        //private work variables
        private StringBuilder _msg = new StringBuilder();
        private StringBuilder _str = new StringBuilder();
        private DatabasePlatform _dbPlatform = DatabasePlatform.MSOracle;
...
#pragma warning restore 1591
        ...
        public PFMsOracle(string dataSource)
        {
        ...
        {
            OracleConnectionStringBuilder odbcConnBuilder = new OracleConnectionStringBuilder(this.ConnectionString);
        ...
        public DataSet RunQueryDataSet(string sqlQuery, CommandType pCommandType)
        {
            ...
            _cmd.CommandType = pCommandType;
            _cmd.CommandTimeout = _commandTimeout;
            if (sqlQuery.ToUpper().Contains("INNER JOIN"))
            {
                string revisedQuery = sqlQuery.ToUpper().Replace("INNER JOIN", "JOIN");
                _cmd.CommandText = revisedQuery;
                _sqlQuery = revisedQuery;
            }
            else
            {
                _cmd.CommandText = sqlQuery;
        ...
        public DataTable RunQueryDataTable(string sqlQuery, CommandType pCommandType)
        {
            ...
            _cmd.CommandType = pCommandType;
            _cmd.CommandTimeout = _commandTimeout;
            //workaround for bug that causes MSOracle provider to fail when processing INNER JOIN syntax in FillSchema
            if (sqlQuery.ToUpper().Contains("INNER JOIN"))
            {
                string revisedQuery = sqlQuery.ToUpper().Replace("INNER JOIN", "JOIN");
                _cmd.CommandText = revisedQuery;
        ...
        public void ImportDataFromDataTable(DataTable dt, int updateBatchSize)
        {
            ...
            //sql.Append(";");  //MS Oracle provider throws invalid character error if it sees the semi-colon terminator
            OracleCommand cmd = new OracleCommand(sql.ToString(), _conn);
            cmd.CommandType = CommandType.Text;
            cmd.CommandTimeout = _commandTimeout;
            DataTable dbTable = dt.Clone();  //Only structure is copied; not the rows, if any.
        ...
        private bool FixColumnDataTypes(DataTable dt, DataTable dbTable)
        {
            ...
            for (int c = 0; c < dt.Columns.Count; c++)
            {
                ...
                if (dt.Columns[c].DataType.FullName == "System.String")
                {
                    if (dt.Columns[c].AllowDBNull == true)
                    {
                        dataTypeFixNeeded = true;
                    }
                }
        ...
        private void FixColumnDataValues(DataTable dt, DataTable dbTable)
        {
            ...
            for (int r = 0; r < dt.Rows.Count; r++)
            {
                ...
                for (int c = 0; c < dt.Columns.Count; c++)
                {
                    ...
                    if (dt.Columns[c].DataType.FullName == "System.String")
                    {
                        if (dt.Columns[c].AllowDBNull == true && dt.Rows[r][c].ToString() == string.Empty)
                        {
                            dr[c] = DBNull.Value;
                        }
                        else
