...
namespace PFInformixObjects
{
    /// <summary>
    /// Class encapsulates Informix .NET provider functionality.
    /// Requires Informix client to be installed on the machine using this class. (e.g. IBM.Data.Informix)
    /// </summary>
    public class PFInformix : IDatabaseProvider
    {

        //private work variables
        private StringBuilder _msg = new StringBuilder();
        private StringBuilder _str = new StringBuilder();
        private DatabasePlatform _dbPlatform = DatabasePlatform.Informix;

        private IfxConnection _conn = new IfxConnection();
        private IfxCommand _cmd = new IfxCommand();
...
#pragma warning restore 1591
        ...
        public PFInformix(string serverName)
        {
        ...
        /// <summary>
        /// Port number for the Informix instance.
        /// </summary>
        ...
        {
            IfxConnectionStringBuilder ifxConnBuilder;
            if (_connectionString.Trim().Length > 0)
            {
                ifxConnBuilder = new IfxConnectionStringBuilder(_connectionString.Trim());
                _serverName = ifxConnBuilder.Server.Trim();
                _databaseName = ifxConnBuilder.Database.Trim();
                _username = ifxConnBuilder.UserID.Trim();
                _password = ifxConnBuilder.Password;
            }
            else
            {
        ...
        {
            IfxConnectionStringBuilder ifxConnBuilder = new IfxConnectionStringBuilder();

            if (this.ServerName.Trim().Length > 0)
            {
                if (this.PortNumber.Trim().Length > 0)
                {
                    ifxConnBuilder.Server = this.ServerName.Trim() + ":" + this.PortNumber.Trim();
                }
                else
                {
                    ifxConnBuilder.Server = this.ServerName.Trim();
                }
            }
            else
                ifxConnBuilder.Server = "localhost:50000";
            if (this.DatabaseName.Trim().Length > 0)
                ifxConnBuilder.Database = this.DatabaseName.Trim();

            if (this.Username.Trim().Length > 0)
                ifxConnBuilder.UserID = this.Username.Trim();
            if (this.Password.Length > 0)
                ifxConnBuilder.Password = this.Password;

            _connectionString = ifxConnBuilder.ToString();
        }
        ...
        private PFCollectionsObjects.PFKeyValueList<string, string> GetConnectionStringKeyVals()
        {
            ...
            foreach (string key in ifxConnBuilder.Keys)
            {
                _connectionStringKeyVals.Add(new stKeyValuePair<string, string>(key, ifxConnBuilder[key].ToString()));
            }
        ...
        public bool TableExists(string tableName)
        {
            ...
            if (names.Length == 2)
            {
                schemaName = names[0].ToLower();
                tabName = names[1].ToLower();
            }
            else
            {
                _msg.Length = 0;
            ...
            sql.Append(" from systables");
            sql.Append(" where tabtype in ('T','V')");
            ...
            rdr = (IfxDataReader)RunQueryDataReader(sql.ToString(), System.Data.CommandType.Text);
            if (rdr.HasRows)
            {
                rdr.Read();
        ...
        public bool DropTable(string tableName)
        {
            bool ret = false;
            StringBuilder sql = new StringBuilder();
            string tabName = tableName.ToLower();
        ...
        public DbDataReader RunQueryDataReader(string sqlQuery, CommandType pCommandType)
        {
            ...
            _sqlQuery = sqlQuery;
            IfxDataReader rdr = _cmd.ExecuteReader();
            return rdr;
        }
        ...
        public int RunNonQuery(string sqlText, CommandType pCommandType)
        {
            ...
            IfxCommand cmd = new IfxCommand(sqlText, _conn);
        ...
        /// <remarks>Use batch size of 1 if sending multiple imports in rapid succession. Otherwise, Informix driver will throw an exception and report that there is an unfinished update that must first complete.</remarks>
        public void ImportDataFromDataTable(DataTable dt, int updateBatchSize)
        {
            ...
            sql.Append(";");
            IfxCommand cmd = new IfxCommand(sql.ToString(), _conn);
            cmd.CommandTimeout = _commandTimeout;
            ...
            IfxCommandBuilder builder = new IfxCommandBuilder(da);
