...
namespace PFConnectionStrings
{
    ...
    /// </summary>
    public partial class SQLAnywhereConnectionStringForm : Form, IConnectionStringForm
    {
        ...
        private string _saveEncryption = string.Empty;
        private string _saveEncryptedPassword = string.Empty;

        //private variables for properties
        DatabasePlatform _dbPlatform = DatabasePlatform.SQLAnywhere;    //TODO: Changed
        ...
        /// Constructor.
        /// </summary>
        public SQLAnywhereConnectionStringForm()
        {
            InitializeComponent();
        }
...
#pragma warning disable 1591
        ...
        private void SaveFormFields()
        {
             ...
             _saveEncryption = this.txtEncryption.Text;
             _saveEncryptedPassword = this.txtEncryptedPassword.Text;
        }
        ...
        private string BuildConnectionString()
        {
            ...
            _db.SetPropertyValue("DataSourceName",this.txtDataSource.Text);
            ...
            _db.SetPropertyValue("Encryption",this.txtEncryption.Text);
            _db.SetPropertyValue("EncryptedPassword",this.txtEncryptedPassword.Text);
        ...
        private void GetDatabaseFileName()
        {
            ...
            _saveFilter = "Database Files|*.db|UltraLite Database Files|*.udb|DataFiles|*txt;*.dat|All Files|*.*";
            DialogResult res = ShowOpenFileDialog();
            if (res == DialogResult.OK)
            {
                this.txtDatabaseFile.Text = _saveSelectionsFile;
                if (_saveSelectionsFile.Contains(@"\DataExports") && _saveSelectionsFile.Contains("ExportData.db"))
                {
        ...
        {
            string sampleDbZipFile = Path.Combine(_desktopDbFolder, "SampleOrderData.db.zip");
            string sampleDbFolder = _desktopDbFolder;
            string sampleDbFile = Path.Combine(_desktopDbFolder, "SampleOrderData.db");

            if (File.Exists(sampleDbFile) == false)
            {
                if (File.Exists(sampleDbZipFile))
                {
                    ZipArchive za = new ZipArchive(sampleDbZipFile);
                    za.ExtractAll(sampleDbFolder);
                    za = null;
                }
            }
        ...
        {
            if (HelpFileExists())
                Help.ShowHelp(this, _helpFilePath, HelpNavigator.KeywordIndex,"SQL Anywhere Connection Strings");
        }
