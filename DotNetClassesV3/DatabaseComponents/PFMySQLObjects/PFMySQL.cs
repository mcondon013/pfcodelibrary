...
namespace PFMySQLObjects
{
    /// <summary>
    /// Class manages data access for MySQL databases using the MySQL .NET Data Provider.
    /// Requires MySQL client to be installed on the machine using this class. (e.g. MySQL.Data)
    /// </summary>
    public class PFMySQL : IDatabaseProvider 
    {
        //private work variables
        private StringBuilder _msg = new StringBuilder();
        private StringBuilder _str = new StringBuilder();
        private DatabasePlatform _dbPlatform = DatabasePlatform.MySQL;
...
#pragma warning restore 1591
        ...
        public PFMySQL(string serverName)
        {
        ...
        {
            MySqlConnectionStringBuilder mysqlConnBuilder;
            if (_connectionString.Trim().Length > 0)
            {
                mysqlConnBuilder = new MySqlConnectionStringBuilder(_connectionString.Trim());
                _serverName = mysqlConnBuilder.Server.Trim();
                _databaseName = mysqlConnBuilder.Database.Trim();
                _username = mysqlConnBuilder.UserID.Trim();
                _password = mysqlConnBuilder.Password;
                _portNumber = mysqlConnBuilder.Port.ToString();
            }
            else
            {
        ...
        private void BuildConnectionString()
        {
            ...
            if (this.ServerName.Trim().Length > 0)
                mysqlConnBuilder.Server = this.ServerName.Trim();
            else
                mysqlConnBuilder.Server = "localhost";

            if (this.PortNumber.Trim().Length > 0)
                mysqlConnBuilder.Port = Convert.ToUInt32(this.PortNumber);
            if (this.DatabaseName.Trim().Length > 0)
                mysqlConnBuilder.Database = this.DatabaseName.Trim();

            if (this.Username.Trim().Length > 0)
                mysqlConnBuilder.UserID = this.Username.Trim();
            if (this.Password.Length > 0)
                mysqlConnBuilder.Password = this.Password;



            _connectionString = mysqlConnBuilder.ToString();
        }
        ...
        private PFCollectionsObjects.PFKeyValueList<string, string> GetConnectionStringKeyVals()
        {
            ...
            foreach (string key in mysqlConnBuilder.Keys)
            {
                _connectionStringKeyVals.Add(new stKeyValuePair<string, string>(key, mysqlConnBuilder[key].ToString()));
            }
        ...
        public bool TableExists(string databaseName, string tableName)
        {
            ...
            schemaName = databaseName.ToUpper();
            tabName = tableName.ToUpper();

            sql.Length = 0;
            sql.Append("select count(*) as numRows");
            sql.Append(" from information_schema.tables");
            sql.Append(" where TABLE_TYPE in ('BASE TABLE','VIEW')");
            ...
            rdr = (MySqlDataReader)RunQueryDataReader(sql.ToString(), System.Data.CommandType.Text);
            if (rdr.HasRows)
            {
                rdr.Read();
        ...
        public bool DropTable(string databaseName, string tableName)
        {
            ...
            string schemaName = databaseName.ToUpper();
        ...
        public int RunNonQuery(string sqlText, CommandType pCommandType)
        {
            ...
            MySqlCommand cmd = new MySqlCommand(sqlText, _conn);
        ...
        /// <summary>
        /// Loads rows contained in an ADO.NET data table to an PFMySQL database table. Table must already exist. See CreateTable methods to create a new table.
        /// </summary>
        ...
        {
            StringBuilder sql = new StringBuilder();
            //make a copy of the data table and change column data types, as needed
            //for example, sybase dataadapter FillSchema has problem with System.Guid type
            ...
            sql.Append(";");
            MySqlCommand cmd = new MySqlCommand(sql.ToString(), _conn);
        ...
        private bool FixColumnDataTypes(DataTable dt, DataTable dbTable)
        {
            ...
            for (int c = 0; c < dt.Columns.Count; c++)
            {
                ...
                else if (dt.Columns[c].DataType.FullName == "System.char[]")
                {
        ...
        private void FixColumnDataValues(DataTable dt, DataTable dbTable)
        {
            ...
            for (int r = 0; r < dt.Rows.Count; r++)
            {
                ...
                for (int c = 0; c < dt.Columns.Count; c++)
                {
                    ...
                    {
                        //dbTable.Columns[c].DataType = System.Type.GetType("System.Byte[]");
                        string str = new string((Char[])dt.Rows[r][c]);
