...
namespace PFSQLServerCE35Objects
{
    /// <summary>
    /// Class to manage access to SQL Server CE 3.5 databases.
    /// Requires SQL SERVER CE 3.5 SP2 to be installed on the machine using this class. (e.g. C:\Program Files (x86)\Microsoft SQL Server Compact Edition\v3.5\Desktop\System.Data.SqlServerCe.dll)
    /// </summary>
    public class PFSQLServerCE35 : IDatabaseProvider, IDesktopDatabaseProvider 
    {
        private StringBuilder _msg = new StringBuilder();
        private StringBuilder _str = new StringBuilder();
        private DatabasePlatform _dbPlatform = DatabasePlatform.SQLServerCE35;

        private int _defaultMaxDatabaseSize = (int)enSQLCE35Defaults.MaxDatabaseSize;
        private int _defaultMaxBufferSize = (int)enSQLCE35Defaults.MaxBufferSize;
        private int _defaultMaxTempFileSize = (int)enSQLCE35Defaults.MaxTempFileSize;

        private string _databasePath = string.Empty;
        private string _databasePassword = string.Empty;
        private bool _encryptionOn = false;
        private SQLCE35EncryptionMode _encryptionMode = SQLCE35EncryptionMode.EngineDefault;
        private int _maxDatabaseSize = (int)enSQLCE35Defaults.MaxDatabaseSize;
        private int _maxBufferSize = (int)enSQLCE35Defaults.MaxBufferSize;
        private int _maxTempFileSize = (int)enSQLCE35Defaults.MaxTempFileSize;
...
#pragma warning restore 1591
        ...
        /// <summary>
        /// Convert string description into a valid SQLCE35EncryptionMode value.
        /// </summary>
        /// <param name="encMode">String name of encryption mode.</param>
        /// <returns>SQLCE35EncryptionMode value.</returns>
        public static SQLCE35EncryptionMode GetEncryptionMode(string encMode)
        {
            SQLCE35EncryptionMode ret = SQLCE35EncryptionMode.EngineDefault;

            switch (encMode)
            {
                case "EngineDefault":
                    ret = SQLCE35EncryptionMode.EngineDefault;
                    break;
                case "PlatformDefault":
                    ret = SQLCE35EncryptionMode.PlatformDefault;
                    break;
                case "PPC2003Compatibility":
                    ret = SQLCE35EncryptionMode.PPC2003Compatibility;
                    break;
                case "Engine Default":
                    ret = SQLCE35EncryptionMode.EngineDefault;
                    break;
        ...
        /// <summary>
        /// Converts SQLCE35EncryptionMode value to a string name.
        /// </summary>
        ...
        public static string GetEncryptionModeDescription(SQLCE35EncryptionMode encMode)
        {
            string ret = "engine default";

            switch (encMode)
            {
                case SQLCE35EncryptionMode.EngineDefault:
                    ret = "engine default";
                    break;
                case SQLCE35EncryptionMode.PlatformDefault:
                    ret = "platform default";
                    break;
                case SQLCE35EncryptionMode.PPC2003Compatibility:
        ...
        public PFSQLServerCE35(string databasePath)
        {
        ...
        public PFSQLServerCE35(string databasePath, string databasePassword)
        {
            this.DatabasePath = databasePath;
        ...
        /// </summary>
        public SQLCE35EncryptionMode EncryptionMode
        {
            get
            {
        ...
        private void ParseConnectionString()
        {
            ...
            _encryptionOn = false;
            _encryptionMode = SQLCE35EncryptionMode.EngineDefault;
            if (_connectionString.Trim().Length > 0)
            {
                ...
                while (kv.Key != string.Empty)
                {
                    ...
                    {
                        val = kv.Value.TrimStart('\'').TrimEnd('\'');
                        _databasePath = val;
                    }
                    else if (kv.Key.ToLower() == "database password")
                    {
                        val = kv.Value.TrimStart('\'').TrimEnd('\'');
                        _databasePassword = val;
                    }
                    else if (kv.Key.ToLower() == "encrypt")
                        _encryptionOn = PFTextProcessor.ConvertStringToBoolean(kv.Value, "false");
                    else if (kv.Key.ToLower() == "encryption mode")
                    {
                        val = kv.Value.TrimStart('\'').TrimEnd('\'');
                        try
                        {
                            _encryptionMode = PFSQLServerCE35.GetEncryptionMode(val);
                        }
                        catch
                        {
                            _encryptionMode = SQLCE35EncryptionMode.Unknown;
                        }
                    }
                    else if (kv.Key.ToLower().Trim() == "max database size")
                    {
                        val = kv.Value.TrimStart('\'').TrimEnd('\'').Trim();
                        _maxDatabaseSize = PFTextObjects.PFTextProcessor.ConvertStringToInt(val, (int)enSQLCE35Defaults.MaxDatabaseSize);
                    }
                    else if (kv.Key.ToLower().Trim() == "max buffer size")
                    {
                        val = kv.Value.TrimStart('\'').TrimEnd('\'').Trim();
                        _maxBufferSize = PFTextObjects.PFTextProcessor.ConvertStringToInt(val, (int)enSQLCE35Defaults.MaxBufferSize);
                    }
                    else if (kv.Key.ToLower().Trim() == "temp file max size")
                    {
                        val = kv.Value.TrimStart('\'').TrimEnd('\'').Trim();
                        _maxTempFileSize = PFTextObjects.PFTextProcessor.ConvertStringToInt(val, (int)enSQLCE35Defaults.MaxTempFileSize);
                    }
                    else
        ...
        private string BuildConnectionString()
        {
            ...
            if (this.DatabasePath.Length > 0)
            {
                ...
                if (this.DatabasePath.StartsWith("'") == false)
                    sb.Append("'");
                sb.Append(this.DatabasePath);
                if (this.DatabasePath.EndsWith("'") == false)
                    sb.Append("'");
                sb.Append(";");
            }
            if (this.DatabasePassword.Length > 0)
            {
                ...
                if (this.DatabasePassword.StartsWith("'") == false)
                    sb.Append("'");
                sb.Append(this.DatabasePassword);
                if (this.DatabasePassword.EndsWith("'") == false)
                    sb.Append("'");
                sb.Append(";");
            }
            if (this.EncryptionOn)
            {
                ...
                if(this.EncryptionMode==SQLCE35EncryptionMode.Unknown)
                    this.EncryptionMode=SQLCE35EncryptionMode.EngineDefault;
                sb.Append(PFSQLServerCE35.GetEncryptionModeDescription(this.EncryptionMode));
                sb.Append("';");
            }
            if (this.MaxDatabaseSize != _defaultMaxDatabaseSize)
            {
                sb.Append("Max Database Size=");
                sb.Append(this.MaxDatabaseSize.ToString());
                sb.Append(";");
            }
            if (this.MaxBufferSize != _defaultMaxBufferSize)
            {
                sb.Append("Max Buffer Size=");
                sb.Append(this.MaxBufferSize.ToString());
                sb.Append(";");
            }
            if (this.MaxTempFileSize != _defaultMaxTempFileSize)
            {
                sb.Append("Temp File Max Size=");
                sb.Append(this.MaxTempFileSize.ToString());
                sb.Append(";");
            }
        ...
        /// <returns>Object representing SQL Server CE 3.5 engine.</returns>
        ...
        /// <summary>
        /// Creates a new SQLCE 3.5 .sdf database file by copying a template .sdf file.
        /// </summary>
        ...
        public void ImportDataFromDataTable(DataTable dt, int updateBatchSize)
        {
            ...
            da.UpdateBatchSize = 1;  //SQLCE throws exception if UpdateBatchSize not equal to 1
            da.Update(dbTable);
