...
namespace TestprogLogManager
{
    public partial class MainForm : Form
    {
        ...
        {
            this.optTextFile.Checked = true;
            this.optRetryXmlFile.Checked = true;

            this.chkShowDateTime.Checked = true;
            this.chkShowMessageType.Checked = false;
            this.chkShowErrorWarningTypes.Checked = true;
            this.chkShowApplicationName.Checked = false;
            this.chkShowMachineName.Checked = false;
            this.chkShowUsername.Checked = false;

            this.txtMessage.Text = "The test message is this. Write this to the log.";
        ...
        private void RunTests()
        {
            ...
            try
            {
                ...
                if (this.chkInitLogWriteTest.Checked)
                {
                    nNumTestsSelected++;
                    Tests.InitLogWriteTest(this);
                }

                if (this.chkInitRetryQueueTest.Checked)
                {
                    nNumTestsSelected++;
                    Tests.InitRetryQueueTest(this);
                }

                if (this.chkWriteFromRetryQueueTest.Checked)
                {
                    nNumTestsSelected++;
                    Tests.WriteFromRetryQueueTest(this);
                }

                if (this.chkWriteEveryOtherFromRetryQueueTest.Checked)
                {
                    nNumTestsSelected++;
                    Tests.WriteEveryOtherFromRetryQueueTest(this);
                }

                if (this.chkWriteMessageToLog.Checked)
                {
                    nNumTestsSelected++;
                    Tests.TestWriteMessageToLog(this);
                }

                if (this.chkWriteMessageToRetryQueue.Checked)
                {
                    nNumTestsSelected++;
                    Tests.TestWriteMessageToRetryQueue(this);
                }
